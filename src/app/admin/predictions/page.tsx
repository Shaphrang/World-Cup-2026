...
export default function AdminPredictionsPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(({ prediction, profile }) => {
      const status = prediction.status || "submitted";
  ...
  const uniqueUsers = new Set(predictions.map((row) => row.user_id)).size;
  const evaluatedCount = predictions.filter((row) => row.is_evaluated).length;
  const submittedCount = predictions.filter((row) => !row.is_evaluated).length;
                        ...
                        <tr key={prediction.id} className="hover:bg-slate-50/80">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <p className="mt-1 text-xs font-bold text-slate-600">
                              {display(prediction.team_a_short_name || prediction.team_a_name)}
                              {" vs "}
                              {display(prediction.team_b_short_name || prediction.team_b_name)}
                            </p>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <div className="space-y-1">
                              ...
                              <p className="text-xs text-slate-500">
                                Score: {Number(prediction.exact_score_points ?? 0)}
                                {" · "}
                                Goals: {Number(prediction.total_goals_points ?? 0)}
                                {" · "}
